...
class Predict_IAB:
	...
	def translate_words(self, recurring_n_words):
		recurring_n_words_original = recurring_n_words.copy()
		...
		return recurring_n_words, recurring_n_words_original

	def score_mapping(self, recurring_n_words, category_list, model_name, recurring_n_words_original = []):
		mapping = dict()
		...
		if (self.language == 'english'):
			...
			for i, idx in enumerate(indices):
				mapping[recurring_n_words_original[i][0]] = {
					'id': category_list[idx][0],
					# 'data': recurring_n_words[i][0],
					# 'table': category_list[idx][1],
					'score': round(scores[i].item(), 4),
					'count': recurring_n_words_original[i][1]
				}
		return mapping
